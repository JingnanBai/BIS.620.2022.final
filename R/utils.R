...
#' @description This function could be use for replacing the id value in
#' original dataset with its meaning
#' @param data the original dataset need to do the replacement. need to be
#' data.frame
...
#' data(diabetic_data)
#' data(ids_map)
#' data_mapped <- data_ids_map(diabetic_data, ids_map)
#' @export
data_ids_map <- function(data, data_map) {
  if (!("column" %in% colnames(data_map)) ||
      !("value" %in% colnames(data_map)) ||
...
#' @description This function could be use for transforming given character/
#' category variables to factor. If the given one is numeric, process would be
#' skipped
...
#' @description This function could be use for transforming the character/
#' category variables in given dataset to factor. If the column is numeric,
...
#' @description This function could be use for transforming ICD-9 codes in
#' dataset to its primary category
#' @param x the original dataset need to do the transformation, need to be
#' data.frame
#' @param data_icd the mapping dataset with ICD-9 codes and its primary
#' category, should have columns (`description`, `code_start`, `code_end`).
#' refer to the sample data `icd9_map` for more information about data format
...
#' @export
encode_icd9 <- function(x, data_icd,
                        maplist = c("diag_1", "diag_2", "diag_3")) {
  if (!("description" %in% colnames(data_icd)) ||
      !("code_start" %in% colnames(data_icd)) ||
  ...
  for (col in maplist) {
    ...
    tempcol <-
      sqldf(paste("select description from x left join data_icd on x.",
                        col, " >= code_start and x.", col, "<= code_end"))[, 1]
    tempcol[grepl("E", x)] <- "External Causes of Injury and Poisoning"
    tempcol[grepl("V", x)] <- "Factors influencing Health Status and Contact
